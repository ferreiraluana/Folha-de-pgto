...
public class PaymentSystem
{
    ...
    private static boolean[] commissioned = new boolean[numberEmployee];
    ...
    private static String[][] nameUNDO = new String[numberEmployee][1000];
    private static String[][] addressUNDO = new String[numberEmployee][1000];
    private static String[][] typeUNDO = new String[numberEmployee][1000];
    private static String[][] paymentMethodUNDO = new String[numberEmployee][1000];
    private static double[][] salaryUNDO = new double[numberEmployee][1000];
    private static double[][] hourPriceUNDO = new double[numberEmployee][1000];
    private static int[][] workedHoursUNDO = new int[numberEmployee][1000];
    private static int[][] extraHoursUNDO = new int[numberEmployee][1000];
    private static String[][] payPeriodUNDO = new String[numberEmployee][1000];
    private static double[][] extraUNDO = new double[numberEmployee][1000];
    private static boolean[][] commissionedUNDO = new boolean[numberEmployee][1000];
    private static boolean[][] unionUNDO = new boolean[numberEmployee][1000];
    private static double[][] unionFeeUNDO = new double[numberEmployee][1000];
    private static int[][] idUNDO = new int[numberEmployee][1000];
    private static int state = 0;
    private static int redo_state = 0;
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println("Available operations: [1]/[2]/[3]/[4]/[5]/[6]/[7]/[8]");
    ...
    private static void saveState()
    {
        ...
        {
            nameUNDO[i][state] = name[i];
            addressUNDO[i][state] = address[i];
            typeUNDO[i][state] = type[i];
            paymentMethodUNDO[i][state] = paymentMethod[i];
            salaryUNDO[i][state] = salary[i];
            hourPriceUNDO[i][state] = hourPrice[i];
            workedHoursUNDO[i][state] = workedHours[i];
            extraHoursUNDO[i][state] = extraHours[i];
            payPeriodUNDO[i][state] = payPeriod[i];
            extraUNDO[i][state] = extra[i];
            commissionedUNDO[i][state] = commissioned[i];
            unionUNDO[i][state] = union[i];
            unionFeeUNDO[i][state] = unionFee[i];
            idUNDO[i][state] = id[i];
        }
        redo_state = state;
        state++;
    ...
    private static void operation8()
    {
        System.out.println("Enter the option: [U]ndo | [R]edo");
        String op = data.nextLine();
        if(op.equals("U") || op.equals("u"))
        {
            int i;
            if((redo_state - 1 < 0)) System.out.println("There is not available Redo to do!");
            redo_state--;
            for (i = 1; i < numberEmployee; i++) {
                name[i] = nameUNDO[i][redo_state];
                address[i] = addressUNDO[i][redo_state];
                type[i] = typeUNDO[i][redo_state];
                paymentMethod[i] = paymentMethodUNDO[i][redo_state];
                salary[i] = salaryUNDO[i][redo_state];
                hourPrice[i] = hourPriceUNDO[i][redo_state];
                workedHours[i] = workedHoursUNDO[i][redo_state];
                extraHours[i] = extraHoursUNDO[i][redo_state];
                payPeriod[i] = payPeriodUNDO[i][redo_state];
                extra[i] = extraUNDO[i][redo_state];
                commissioned[i] = commissionedUNDO[i][redo_state];
                union[i] = unionUNDO[i][redo_state];
                unionFee[i] = unionFeeUNDO[i][redo_state];
                id[i] = idUNDO[i][redo_state];
    ...
    {
        //it works only for 21st century
